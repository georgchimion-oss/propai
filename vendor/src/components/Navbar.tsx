...
import { Package, Menu, X, ChevronDown, ExternalLink } from 'lucide-react'
...
const moduleLinks = [
  ...
  { name: 'Docs', url: 'https://vestiadocs.georg.miami' },
  { name: 'Vendor', url: '#', current: true },
]
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-orange/15 flex items-center justify-center border border-orange/25 group-hover:bg-orange/25 transition-colors">
          ...
          <div className="hidden sm:flex items-center gap-3">
            ...
            <div className="relative" ref={modulesRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-muted-fg hover:text-foreground bg-black/4 border border-border hover:border-orange/20 transition-all"
              >
                Modules
                <ChevronDown
              ...
              {modulesOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-card border border-border rounded-xl shadow-xl shadow-black/12 overflow-hidden z-50">
                  {moduleLinks.map((mod) => (
                    <div key={mod.name}>
                      {mod.current ? (
                        <div className="flex items-center justify-between px-3 py-2.5 bg-orange/10">
                          <span className="text-sm font-medium text-orange">{mod.name}</span>
                          <span className="text-[10px] text-orange/70 bg-orange/10 px-1.5 py-0.5 rounded-md font-medium">
                            Active
                          </span>
                        </div>
                      ) : (
            ...
            <div className="mt-2 pt-2 border-t border-border">
              ...
              {moduleLinks.map((mod) =>
                mod.current ? (
                  <div
                    key={mod.name}
                    className="px-4 py-2 text-sm font-medium text-orange flex items-center gap-2"
                  >
                    <span>{mod.name}</span>
                    <span className="text-[10px] bg-orange/10 px-1.5 py-0.5 rounded">Active</span>
                  </div>
                ) : (
                  <a
                    key={mod.name}
