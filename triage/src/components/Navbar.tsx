    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          >
            <div className="w-8 h-8 rounded-lg bg-gold/15 flex items-center justify-center border border-gold/25 group-hover:bg-gold/25 transition-colors">
          ...
          <div className="hidden sm:flex items-center gap-3">
            {/* Desktop Tabs */}
            <div className="flex items-center gap-1 bg-black/5 rounded-xl p-1 border border-border">
              {tabs.map((tab) => (
                <button
                  key={tab.key}
            ...
            <div className="relative" ref={modulesRef}>
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium text-muted-fg hover:text-foreground bg-black/5 border border-border hover:border-gold/20 transition-all"
              >
                Modules
                <ChevronDown
              ...
              {modulesOpen && (
                <div className="absolute right-0 top-full mt-2 w-44 bg-card border border-border rounded-xl shadow-xl shadow-gray-300/60 overflow-hidden z-50">
                  {moduleLinks.map((mod) => (
                    <div key={mod.name}>
                      {mod.current ? (
                        <div className="flex items-center justify-between px-3 py-2.5 bg-gold/10">
                          <span className="text-sm font-medium text-gold">{mod.name}</span>
                          <span className="text-[10px] text-gold/70 bg-gold/10 px-1.5 py-0.5 rounded-md font-medium">
                            Active
                          </span>
                        </div>
                      ) : mod.soon ? (
                        <div className="flex items-center justify-between px-3 py-2.5 opacity-40 cursor-not-allowed">
                          ...
                          <span className="text-[10px] text-muted-fg bg-black/5 px-1.5 py-0.5 rounded-md">
                            Soon
                          </span>
                        </div>
                      ) : (
            ...
            <div className="mt-2 pt-2 border-t border-border">
              ...
              {moduleLinks.map((mod) =>
                mod.current ? (
                  <div
                    key={mod.name}
                    className="px-4 py-2 text-sm font-medium text-gold flex items-center gap-2"
                  >
                    <span>{mod.name}</span>
                    <span className="text-[10px] bg-gold/10 px-1.5 py-0.5 rounded">Active</span>
                  </div>
                ) : mod.soon ? (
                  <div
                    key={mod.name}
                  ...
                  >
                    <span>{mod.name}</span>
                    <span className="text-[10px] bg-black/5 px-1.5 py-0.5 rounded">Soon</span>
                  </div>
                ) : (
                  <a
                    key={mod.name}
